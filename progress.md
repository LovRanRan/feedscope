...
| Field                | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Current Commit**   | `Commit 6 — CLI 整合 + README(in progress · 即将开始)`                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **Overall Progress** | 🟩🟩🟩🟩🟩🟦⬜ 5 / 7 commits done                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Blocker**          | none                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Last Activity**    | 2026-04-29 · **Commit 5 关闭** ⭐ **关键里程碑达成**。BaseFetcher 加 `source: ClassVar[SourceName]` + ArxivFetcher/HNFetcher 加 `source = "..."` + 5 处 `source=self.source` DRY 重构 + orchestrator (`asyncio.gather` + `return_exceptions=True` + isinstance 兜底 + Sequence 协变) + exporter (`model_dump(mode="json")` + `json.dumps(indent=2)`) + test_orchestrator (FakeFetcher + 3 tests:并发 / 合并 / 异常隔离) — 全 hand-written。**12 tests PASSED** + mypy clean + ruff clean + git commit 已打。**Acceptance "asyncio for at least 1 concurrent operation" ✓ 满足**。 |
| **Working Mode**     | HAND-WRITE · resume-grade(Project 0 Commit 2+ 严格执行 — code 全部 Haichuan 写,Claude 只 review + 方向提示)|进度:完成驱动,非日历驱动                                                                                                                                                                                                                                                                                                                                |
| **Next Action**      | Commit 6 — CLI 整合 + README:(1) 重写 `src/feedscope/cli.py` —— 从 Commit 1 占位升级到真 `fetch` sub-command(`feedscope fetch --query "..." --sources arxiv,hn --limit 20 --output out.json`),把 typer 参数转 FetchConfig,创建 httpx.AsyncClient,从 FETCHERS 注册表实例化 fetcher,跑 orchestrator,write_json。(2) `tests/test_cli.py` 用 `typer.testing.CliRunner` + 1-2 个 e2e 测试。(3) `README.md` 填实际内容(install / usage 真 example / architecture / development)。 |
...
### Commit 6 — CLI 整合 + README 🟧 in progress
